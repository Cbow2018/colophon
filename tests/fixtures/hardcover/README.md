...
CBO-41's key for a spelling is `matching.normalise`'s output, and `L.J. Ross`
(318638), `L. J. Ross` (350233) and `LJ Ross` (350235) all come out as
`lj ross`. The spelling that keys away from the other two is
`comparison_text`'s — it leaves `J.` and `R.` as separate tokens, so it reads
`LJ Ross` as `lj ross` and the other two as `l j ross` — and that reader is for
comparison only, never a key anything is stored under. So **`[authors]` in
`config.toml` is the only merge for spellings that `normalise` keeps apart**,
and CBO-41's test says so rather than implying the id solves it.
