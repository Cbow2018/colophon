...
| `audio-edition-by-isbn.json` | `AudioEditionTests.test_an_audio_edition_is_an_isbn_hardcover_does_not_have` | The same Edition as an ISBN reply rather than a title reply: the widened `QUERY`'s fields for `9781799729945`, out of `by-isbn-9781799729945-genres.json` as it stood before that recording was superseded, with `reading_format_id: 2`. The case is Q10's: an ISBN Hardcover lists only as an Audio Edition is an ISBN it does not have. |
